export default function LeaderBoard({ bidderList }: { bidderList: [string, number][] }) {
	return (
		<div className="bg-gradient-to-tr from-white/5 via-white/5 h-fit to-white/10 backdrop-blur-md rounded-lg shadow-lg overflow-hidden mt-6 max-h-[77vh]">
				...
				{bidderList.map((e, ind) => {
					return (
						<li key={ind} className="flex justify-between items-center p-4">
							...
							<span className="text-green-400 font-bold">{e[1]} Million €</span>
						</li>
					);
				})}
			</ul>
